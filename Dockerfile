...
RUN apt-get update \
    ...
    && apt install -y ros-humble-perception-pcl \
  	...
  	&& apt install -y ros-humble-xacro ros-humble-gazebo-ros nautilus\
...
RUN apt-get update \
    && apt-get install -y terminator libeigen3-dev ros-humble-grid-map

SHELL ["/bin/bash", "-c"]

RUN mkdir -p ~/ros2_ws/src /root/.config/terminator /root/launchproject /root/.rviz2/
COPY Utilities/terminator       /root/.config/terminator
COPY Utilities/rvizconfig       /root/.rviz2/
