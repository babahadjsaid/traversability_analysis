...
class TraversabilityAnalysis : public ParamServer{
 public:
    ...
    * Constructor.
    */
    explicit TraversabilityAnalysis(std::shared_ptr<rclcpp::Node> nodeHandle);
    void InitMapLayers();
    ...
    void PcaAnalysis(Cluster &cluster, float &is_vertical, float &ratioBigToSmallest, float &ratioBigTomiddle);
 ...
 private:
    //Sync
    std::mutex mapMtx_,globalMapMtx_;
