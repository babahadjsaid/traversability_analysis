      ...
      for (size_t j = 0; j < size_(1); j++)
        {
          ...
          for (const auto& relIdx : relativeIndices) {
                  ...
                  globalCostmap_->UpdateCell(pos, occupancy,relIdx.z());
              }
        }
   }
...
std::vector<Eigen::Vector3d> OccupancyGrid::getRelativeEllipseIndices(Eigen::Matrix2d covarianceMatrix) {
    ...
    Eigen::Array2d lengths =  2 * uncertaintyFactor * Eigen::Array2d(eigenvalues(maxEigenvalueIndex), eigenvalues(minEigenvalueIndex)).sqrt() ;
    double theta = atan2(solver.eigenvectors().col(maxEigenvalueIndex).real()(1), solver.eigenvectors().col(maxEigenvalueIndex).real()(0));
...
void OccupancyGrid::UpdateCell(Position pos, int8_t cost,double weight){
  ...
  double NUM_UPDATE = 8;
  int8_t newCost = (int) ((weight*oldcost + NUM_UPDATE*oldweight * cost)/(NUM_UPDATE*oldweight + weight));
...
int OccupancyGrid::GetIndexWorldPos(Position pos){
  ...
  if (i>=0 && i < Map_->info.width && j>=0 && j < Map_->info.height) return j*Map_->info.width + i;
  return -2;
}
...
int8_t OccupancyGrid::GetCost(Position pos){
  ...
  if (index>-1) return Map_->data[index];
  return -2;
}
...
void OccupancyGrid::SetCost(Position pos, int8_t value){
  ...
  MapMtx_.lock();
  int index = GetIndexWorldPos(pos);
  if (index>-1) Map_->data[index] = value;
    ...
    for (size_t i = 0; i < Map_->info.width; i++)
      {
        ...
        tmpVar((j + incrY) , i + incrX) = Variance_(j,i);
      }
  }
  delete Map_;
  //delete &Variance_;
