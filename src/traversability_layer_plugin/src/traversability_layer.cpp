...
#include "traversability_layer_plugin/traversability_layer.hpp"
...
PLUGINLIB_EXPORT_CLASS(traversability_layer_plugin::TraversabilityLayer, nav2_costmap_2d::Layer)
...
namespace traversability_layer_plugin
{
...
TraversabilityLayer::onInitialize()
{
  ...
  map_sub_ = node->create_subscription<nav2_msgs::msg::Costmap>(
    map_topic_, map_qos,
...
void
TraversabilityLayer::activate()
{
}

void
TraversabilityLayer::deactivate()
{
...
void
TraversabilityLayer::getParameters()
{
...
void
TraversabilityLayer::processMap(const nav2_msgs::msg::Costmap & new_map)
{
  RCLCPP_DEBUG(logger_, "TraversabilityLayer: Process map");

  unsigned int size_x = new_map.metadata.size_x;
  unsigned int size_y = new_map.metadata.size_y;

  RCLCPP_DEBUG(
    logger_,
    "TraversabilityLayer: Received a %d X %d map at %f m/pix", size_x, size_y,
    new_map.metadata.resolution);

  // resize costmap if size, resolution or origin do not match
  Costmap2D * master = layered_costmap_->getCostmap();
  if (!layered_costmap_->isRolling() && (master->getSizeInCellsX() != size_x ||
    master->getSizeInCellsY() != size_y ||
    master->getResolution() != new_map.metadata.resolution ||
    master->getOriginX() != new_map.metadata.origin.position.x ||
    master->getOriginY() != new_map.metadata.origin.position.y ||
    !layered_costmap_->isSizeLocked()))
  {
    // Update the size of the layered costmap (and all layers, including this one)
    RCLCPP_DEBUG(
      logger_,
      "TraversabilityLayer: Resizing costmap to %d X %d at %f m/pix", size_x, size_y,
      new_map.metadata.resolution);
    layered_costmap_->resizeMap(
      size_x, size_y, new_map.metadata.resolution,
      new_map.metadata.origin.position.x,
      new_map.metadata.origin.position.y,
      true);
  } else if (size_x_ != size_x || size_y_ != size_y ||  // NOLINT
    resolution_ != new_map.metadata.resolution ||
    origin_x_ != new_map.metadata.origin.position.x ||
    origin_y_ != new_map.metadata.origin.position.y)
  {
    // only update the size of the costmap stored locally in this layer
    RCLCPP_INFO(
      logger_,
      "TraversabilityLayer: Resizing static layer to %d X %d at %f m/pix", size_x, size_y,
      new_map.metadata.resolution);
    resizeMap(
      size_x, size_y, new_map.metadata.resolution,
      new_map.metadata.origin.position.x, new_map.metadata.origin.position.y);
  }
...
unsigned char
TraversabilityLayer::interpretValue(unsigned char value)
{
  // check if the static value is above the unknown or lethal thresholds
  if (track_unknown_space_ && value == 255) {
    return NO_INFORMATION;
  } else if (!track_unknown_space_ && value == 255) {
    return FREE_SPACE;
  } else if (value >= 254) {
    return LETHAL_OBSTACLE;
  } 

  double scale = static_cast<double>(value) / 252;
  return scale * LETHAL_OBSTACLE;
}

void
TraversabilityLayer::incomingMap(const nav2_msgs::msg::Costmap::SharedPtr new_map)
{
  ...
  int min_i, int min_j, int max_i, int max_j)
{
  ...
  if (!layered_costmap_->isRolling()) {
    ...
    try {
      ...
      RCLCPP_ERROR(logger_, "TraversabilityLayer: %s", ex.what());
      return;
    }
